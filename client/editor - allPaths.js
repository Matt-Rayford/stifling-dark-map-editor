...
const SpaceType = {
    Basic: {name: 'Basic Movement Space', fontColor: '#000000'},
    Item: {name: 'Item Space', image: 'item-space', fontColor: '#FFFFFF'},
    Objective: {name: 'Objective Item', image: 'objective-space', fontColor: '#FFFFFF'},
    Adversary: {name: 'Adversary', image: 'adversary-space', fontColor: '#FFFFFF'},
	Starting: {name: 'Starting', image: 'start-space', fontColor: '#000000'},
	Interaction: {name: 'Interaction', image: 'interaction-space', fontColor: '#FFFFFF'},
	Escape: {name: 'Escape', image: 'escape-space', fontColor: '#FFFFFF'},
	Involved: {name: 'Involved Action Space', image: 'involved-space', fontColor: '#FFFFFF'},
	Door: {name: 'Door', image: 'door-space', fontColor: '#000000'}
}
...
class Space{
	...
	constructor(x, y, type, number, row, col){
		this.id = id;
	...
	getConnectionsHtml(){
		var conString = '';
		this.connections.sort(function(a, b) { return a.id < b.id ? -1 : 1 });
		for(let i=0; i<this.connections.length; i++)
			conString += '<li>' + this.connections[i].number + 
			' <button type="button" onclick="deleteConnection(' + this.id + ', ' + this.connections[i].id + ', true)">X (<->)</button>' +
			' <button type="button" onclick="deleteConnection(' + this.id + ', ' + this.connections[i].id + ', false)">X (->)</li>';
		return conString;
	}
}
		...
		if(!highlightedObject){
			for(let i=0; i<allObjects.length; i++){
				...
				if(curObject.objectType == ObjectType.Space){
					if(	event.offsetX > curObject.center.x - curObject.radius &&
			...
			if(	event.offsetX > highlightedObject.center.x - highlightedObject.radius &&
				event.offsetX < highlightedObject.center.x + highlightedObject.radius &&
				event.offsetY > highlightedObject.center.y - highlightedObject.radius &&
				event.offsetY < highlightedObject.center.y + highlightedObject.radius)
				{
					// do nothing.
				}
...
canvas.addEventListener('mousedown',function(event){
	if(highlightedObject){
		...
		document.getElementById("info").innerHTML = selectedObject.getHtmlInfo();
		...
		if(selectingPointA){
			...
			document.getElementById("shortestLength").innerHTML = pathInfo.length;
			document.getElementById("pathText").innerHTML = pathInfo.pathString;
		}
		...
		switch(selectedObject.objectType){
			case ObjectType.Space:
				for(var c=0; c<selectedObject.connections.length; c++){
					...
					conSpace.connections = conSpace.connections.filter(s => s.id != selectedObject.id);
				}
...
function renumberSpaces(startNum){
	...
	for(var i=0; i<allSpaces.length; i++){
		...
		if(curSpace.number > startNum)
			curSpace.number--;
	}
}
function deleteConnection(spaceId, connectionId, bothWays){
	var origSpace = spaceMap.get(spaceId);
	origSpace.connections = origSpace.connections.filter(c => c.id != connectionId);
	if(bothWays){
		var connectedSpace = spaceMap.get(connectionId);
		connectedSpace.connections = connectedSpace.connections.filter(c => c.id != spaceId);
	}
}
...
function drawSpaces(){
	...
	for(var i=0; i<arr.length; i++){
		...
		if(space.spaceType.image)
			drawImage(space.center.x - space.radius, space.center.y - space.radius, imageMap.get(space.spaceType.image));
		drawCircle(space.center.x, space.center.y, circleRadius);
		drawText(space.center.x, space.center.y+5, space.number, 'bold 10pt Arial', space.spaceType.fontColor);
		...
		for(let c=0; c<space.connections.length; c++){
			...
			var endX = conSpace.center.x + space.radius*Math.cos(Math.PI + angle);
			var endY = conSpace.center.y + space.radius*Math.sin(Math.PI + angle);
		...
		if(showAllPaths){
			...
			drawText(space.center.x, space.center.y - 20, distance, 'bold 7pt Arial');
			ctx.restore();
		}
	}
	...
	if(drawOneWay || drawTwoWay){
		...
		var angle = Math.atan2(dY, dX);
		var startX = linkStart.center.x + linkStart.radius*Math.cos(Math.PI + angle);
		var startY = linkStart.center.y + linkStart.radius*Math.sin(Math.PI + angle);
		drawArrow(startX, startY, mousePos.x, mousePos.y);
		if(drawTwoWay)
			drawArrow(mousePos.x, mousePos.y, startX, startY);
	}
}
...
function setupSpaces(){
	...
	for(var i=0; i<rows; i++){
		for(var j=0; j<cols; j++){
			var space;
			if(i % 2 == 0)
				space = new Space(j*circleRadius+j*padding+40, i*circleRadius+i*padding+40, SpaceType.Basic, curSpace, i+1, j+1);
			else
				space = new Space(j*circleRadius+j*padding+70, i*circleRadius+i*padding+40, SpaceType.Basic, curSpace, i+1, j+1);
	...
	for(var i=0; i<spaceMap.size; i++){
		...
		if(space.row != 1){
			if(space.row % 2 == 0 || space.col != 1)
				space.connections.push(spaceMap.get(i+topLeftMod-offset));
			if(space.row % 2 == 1 || space.col != cols)
				space.connections.push(spaceMap.get(i+topRightMod-offset));
		}
		
		// Check bottom left and right, if row not at bottom
		if(space.row != rows){
			if(space.row % 2 == 0 || space.col != 1)
				space.connections.push(spaceMap.get(i+bottomLeftMod-offset));
			if(space.row % 2 == 1 || space.col != cols)
				space.connections.push(spaceMap.get(i+bottomRightMod-offset));
		}
		...
		while(visitList.length > 0){
			...
			if(!visitedMap.has(curSpace.id))
			{
				...
				for(var j=0; j<curSpace.connections.length; j++){
					var conSpace = curSpace.connections[j];
					if(!visitedMap.has(conSpace.id))
						visitList.push({space: conSpace, distance: curDistance+1});
				}
			}
		}
...
setupImages();
setupSpaces();
//spaceMap.get(0).spaceType = SpaceType.Item;
redraw();